...
// [Jules: SDK] This crate provides a mock implementation of the Google Cloud Storage gRPC service.
// [Jules: SDK] It allows testing the client library against a local, controllable server instead of the real API.
...
/// ```
// [Jules: Rust] Re-export `MockStorage` to make it available to users of this crate.
...
/// # Ok(()) }
/// ```
// [Jules: Rust] `start` is an async function that starts the gRPC server.
// [Jules: Rust] `T` is a generic type implementing the `Storage` trait (the service definition).
// [Jules: Rust] It returns a tuple containing the bound address (as a String) and a `JoinHandle` to the server task.
pub async fn start<T>(address: &str, service: T) -> anyhow::Result<(String, JoinHandle<()>)>
where
    T: google::storage::v2::storage_server::Storage,
{
    // [Jules: Rust] Bind a TCP listener to the specified address. Using "0.0.0.0:0" will bind to a random available port.
    let listener = tokio::net::TcpListener::bind(address).await?;
    // [Jules: Rust] Get the actual address the listener was bound to (useful if port was 0).
    let addr = listener.local_addr()?;

    // [Jules: Rust] Spawn the server loop in a background task.
    let server = tokio::spawn(async {
        // [Jules: Rust] Create a stream of incoming connections from the listener.
        let stream = tokio_stream::wrappers::TcpListenerStream::new(listener);

        // [Jules: Rust] Build and run the Tonic gRPC server.
        let _ = tonic::transport::Server::builder()
            // [Jules: SDK] Register the Storage service.
            .add_service(google::storage::v2::storage_server::StorageServer::new(
                service,
            ))
            // [Jules: Rust] Serve using the incoming connection stream.
            .serve_with_incoming(stream)
            .await;
    });

    // [Jules: Rust] Return the URI and the task handle.
    Ok((to_uri(addr), server))
}

// [Jules: Rust] Helper function to format a SocketAddr as a URI string.
...
// [Jules: Rust] This module organizes the generated protobuf code.
// [Jules: Rust] `#[allow(clippy::large_enum_variant)]` suppresses a specific Clippy lint warning often triggered by generated code.
#[allow(clippy::large_enum_variant)]
pub mod google {
    pub mod iam {
        pub mod v1 {
            // [Jules: Rust] `include!` macro inserts the contents of the specified file here.
...
mod tests {
    ...
    // [Jules: SDK] Tests that the mock server can handle server-streaming RPCs (ReadObject).
    #[tokio::test]
    async fn mock_stub_server_streaming_success() -> anyhow::Result<()> {
        let (tx, rx) = tokio::sync::mpsc::channel(1);
        let mut mock = MockStorage::new();
        // [Jules: SDK] Configure the mock to return the receiver side of a channel as the stream.
        ...
        // [Jules: Rust] Send an error through the channel to simulate a stream error.
    ...
    // [Jules: SDK] Tests that the mock server can handle client-streaming RPCs (BidiReadObject).
    #[tokio::test]
    async fn mock_stub_client_streaming_success() -> anyhow::Result<()> {
        ...
        let mut mock = MockStorage::new();
        // [Jules: SDK] Configure the mock to capture the incoming request stream.
        ...
        // [Jules: Rust] Send some requests to the client stream.
        for i in 0..3 {
    ...
    // [Jules: Rust] Macros to generate tests for bidirectional streaming methods.
    ...
    // [Jules: Rust] Macros for client streaming tests.
    ...
    // [Jules: Rust] Macros for unary (non-streaming) tests.
