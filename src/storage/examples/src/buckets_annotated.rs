...
// [Jules: Rust]
// This file acts as a "manifest" for the `buckets` module.
// It lists all the sub-modules (which correspond to individual files in the `buckets/` directory).
// By declaring them `pub mod`, we make them accessible to users of this crate.

pub mod add_bucket_conditional_iam_binding;
// [Jules: Rust]
// `#[allow(dead_code)]` suppresses compiler warnings about unused code.
// This is often used in example code or when a module is not yet fully integrated or used by the main library,
// but we want to keep it compiled to ensure it remains valid.
#[allow(dead_code)]
