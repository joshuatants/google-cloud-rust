...
pub async fn sample(
    ...
    // [Jules: SDK]
    // Copying objects in Google Cloud Storage is done via "rewrite".
    // `rewrite_object` handles both copying and potentially transcoding (changing encryption, storage class, etc.).
    let copied = client
        .rewrite_object()
        ...
        // [Jules: SDK]
        // `rewrite_until_done()` is an extension method provided by `RewriteObjectExt`.
        // Rewriting large objects can take multiple RPC calls. This helper loops until the copy is complete.
        // Without this, you might only copy a portion of the object.
        .rewrite_until_done()
        .await?;
    println!(
