...
// [Jules: SDK]
// Data integrity is critical in cloud storage.
// This enum lists the different ways a checksum validation can fail.
// - `Crc32c`: The primary checksum used by Google Cloud Storage.
// - `Md5`: An alternative hash, often used for legacy compatibility or additional verification.
...
// [Jules: Rust]
// `#[derive(Clone, Debug)]` automatically implements the `Clone` and `Debug` traits for this enum.
// `#[non_exhaustive]` means that new variants might be added in the future.
// This forces users to match with a wildcard `_` arm, preventing their code from breaking if we add a new variant.
#[derive(Clone, Debug)]
#[non_exhaustive]
pub enum ChecksumMismatch {
    ...
    // [Jules: Rust]
    // We use `Box<ObjectChecksums>` here to reduce the size of the enum variant.
    // If we embedded `ObjectChecksums` directly, this variant might be very large,
    // making the whole enum large (since enums are sized by their largest variant).
    // `Box` puts the data on the heap, so the enum only stores a pointer.
    Both {
...
// [Jules: Rust]
// We implement `std::fmt::Display` to provide a user-friendly error message.
// This is required for types that want to implement `std::error::Error`.
...
// [Jules: Rust]
// `#[derive(thiserror::Error)]` uses the `thiserror` crate to automatically implement `std::error::Error`.
// It uses the `#[error(...)]` attributes on variants to generate the `Display` implementation.
...
// [Jules: Rust]
// A type alias for a boxed error object that is thread-safe (`Send + Sync`).
// `'static` means it doesn't contain any non-static references.
...
pub enum ReadError {
    // [Jules: Rust]
    // The `#[error(...)]` string can use format syntax like `{0}` to refer to the first field of the tuple variant.
...
// [Jules: Rust]
// `#[error(transparent)]` delegates the error message to the inner error source.
// This is useful when the error is just a wrapper around another error type.
#[derive(thiserror::Error, Debug)]
#[error(transparent)]
...
// [Jules: Rust]
// This internal enum hides the implementation details of `SigningError`.
// `SigningError` wraps this enum but doesn't expose it publicly, allowing us to change it later without breaking the public API.
#[derive(thiserror::Error, Debug)]
#[allow(dead_code)]
