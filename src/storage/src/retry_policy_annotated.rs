...
// [Jules: SDK]
// This function returns the default retry configuration used by the client.
// It combines the standard `RetryableErrors` logic with a 300-second (5 minute) total time limit.
...
// [Jules: SDK]
// This struct implements the core logic for deciding which errors should trigger a retry.
// It doesn't handle *how* to retry (backoff, time limits), just *what* to retry.
...
impl RetryPolicy for RetryableErrors {
    fn on_error(&self, state: &RetryState, error: Error) -> RetryResult {
        // [Jules: SDK]
        // Some errors happen before the request even reaches the server (e.g., DNS failure).
        // These are always safe to retry because the server hasn't seen the request yet.
        if error.is_transient_and_before_rpc() {
            return RetryResult::Continue(error);
        }
        // [Jules: SDK]
        // Ideally, we only retry idempotent operations (operations that can be repeated safely).
        // If an operation is not idempotent, retrying might cause side effects (e.g., charging a credit card twice).
        if !state.idempotent {
            return RetryResult::Permanent(error);
        }
        // [Jules: SDK]
        // I/O errors (network connection dropped) and timeouts are generally safe to retry.
        if error.is_io() || error.is_timeout() {
            return RetryResult::Continue(error);
        }
        // [Jules: SDK]
        // Sometimes gRPC transport errors don't map cleanly to HTTP status codes.
...
// [Jules: SDK]
// 308 Resume Incomplete is a special status code for resumable uploads.
// It's not an error in the traditional sense; it just means "I got some data, but I need more".
// We wrap the standard policy to handle this specific case.
...
// [Jules: Rust]
// Implementing `RetryPolicy` for our decorator.
// We use `Arc<dyn RetryPolicy>` for the inner type to allow flexibility.
impl RetryPolicy for ContinueOn308<Arc<dyn RetryPolicy + 'static>> {
    fn on_error(&self, state: &RetryState, error: Error) -> RetryResult {
        if error.http_status_code() == Some(308) {
            return RetryResult::Continue(error);
        }
        // Delegate to the inner policy for all other errors.
