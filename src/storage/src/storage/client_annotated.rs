...
// [Jules]
// `use` brings items from other modules into scope so we can use them without
// fully qualifying their names (e.g. `crate::Error` instead of `google_cloud_storage::Error`).
//
// `super::` refers to the parent module.
// `crate::` refers to the root of the current crate (`google_cloud_storage`).
...
// [Jules]
// `#[derive(Clone, Debug)]` automatically implements the `Clone` and `Debug` traits for this struct.
// - `Clone` allows us to create a copy of the client. Since the client internals are wrapped in `Arc` (see below),
//   cloning is cheap (just incrementing a reference count) and does not duplicate the underlying connections.
// - `Debug` allows us to format the struct using `{:?}` for logging/debugging.
//
// `pub struct Storage<S = crate::stub::DefaultStorage>` defines a public struct named `Storage`.
// `<S = ...>` defines a generic type parameter `S`.
// If the user doesn't specify `S`, it defaults to `crate::stub::DefaultStorage`.
// This pattern allows us to swap out the backend implementation (e.g., for testing/mocking)
// while providing a sensible default for normal users.
//
// `where S: ...` specifies "trait bounds". It forces `S` to implement the `crate::stub::Storage` trait
// and have a `'static` lifetime (meaning it doesn't hold any temporary references).
#[derive(Clone, Debug)]
...
{
    // [Jules]
    // `std::sync::Arc` stands for "Atomically Reference Counted".
    // It's a smart pointer that allows shared ownership of data across multiple threads.
    // When we clone `Storage`, we just clone this Arc, pointing to the same `stub`.
    // The `stub` is what actually performs the operations (network calls).
    stub: std::sync::Arc<S>,
    options: RequestOptions,
}

// [Jules]
// `pub(crate)` makes this struct visible only within this crate. It's an internal implementation detail.
// `StorageInner` holds the actual HTTP and gRPC clients and credentials.
// The `Storage` struct above wraps this `StorageInner` (indirectly via `DefaultStorage` which holds `StorageInner`).
#[derive(Clone, Debug)]
pub(crate) struct StorageInner {
    // [Jules]
    // `reqwest` is a popular Rust HTTP client. `gaxi` seems to be an internal wrapper/alias.
...
// [Jules]
// `impl Storage` defines methods associated with the `Storage` struct.
// This block contains methods that don't depend on the generic parameter `S` (or use the default).
impl Storage {
...
// [Jules]
// This implementation block applies to `Storage<S>` for ANY type `S` that implements `crate::storage::stub::Storage`.
impl<S> Storage<S>
where
    S: crate::storage::stub::Storage + 'static,
{
    ...
    // [Jules]
    // This method uses generics heavily (`<B, O, T, P>`).
    // `B: Into<String>`: `bucket` can be anything that can be converted into a String (e.g. `&str`, `String`).
    // `T: Into<Payload<P>>`: `payload` can be converted into a `Payload`. This allows passing various types like strings, bytes, or files seamlessly.
    //
    // It returns a `WriteObject` struct, which is a "Builder".
    // The user calls this method to start constructing a request, but the request isn't sent until they call `.send_buffered()` or `.send_unbuffered()` on the returned `WriteObject`.
...
// [Jules]
// This impl block defines methods for `Storage` (implicitly `Storage<DefaultStorage>`).
impl Storage {
    // [Jules]
    // `pub(crate)` means this is an internal constructor used by the builder.
    // `async fn` means this function returns a `Future`. It performs asynchronous operations (like establishing connections).
    // `BuilderResult<Self>`: It returns a `Result` which is either the new `Storage` instance or an error.
    pub(crate) async fn new(builder: ClientBuilder) -> BuilderResult<Self> {
        // `StorageInner::from_parts` consumes the builder to create the inner client components.
        let inner = StorageInner::from_parts(builder).await?;
        let options = inner.options.clone();
        // `crate::storage::transport::Storage` is the concrete implementation of the `Storage` trait (the default stub).
...
impl StorageInner {
    ...
    // [Jules]
    // `pub(self)` is equivalent to `private` in other languages. This method is only callable from within `StorageInner`.
    pub(self) fn new(
    ...
    pub(self) async fn from_parts(builder: ClientBuilder) -> BuilderResult<Self> {
        // [Jules]
        // `builder.into_parts()?`: The `?` operator is crucial in Rust error handling.
        // It means "if the result is Ok, give me the value. If it's Err, return the error from this function immediately."
    ...
    pub async fn apply_auth_headers(
        ...
        let auth_headers = match cached_auth_headers {
            ...
            CacheableResource::NotModified => {
                // [Jules]
                // `unreachable!` is a macro that panics if executed. It tells the compiler (and readers)
                // "logic dictates this code path should never be taken".
...
/// ```
// [Jules]
// The "Builder Pattern" is very common in Rust for constructing complex objects with optional configuration.
// Instead of a constructor with 10 arguments (many optional), we use a Builder struct with fluent methods
// (like `.with_endpoint(...)`) to set options one by one.
...
impl ClientBuilder {
    pub(crate) fn new() -> Self {
        // [Jules]
        // `ClientConfig::default()` creates a configuration with sensible default values.
        ...
        {
            // [Jules]
            // We optimize the number of gRPC channels based on the number of CPU cores available.
    ...
    // [Jules]
    // `async fn build(self)`:
    // 1. `async`: Creating the client might involve network I/O (e.g. fetching credentials).
    // 2. `self`: This consumes the builder. You can't use the builder again after calling `build()`.
...
// [Jules]
// `#[cfg(test)]` means this module is only compiled when running tests (e.g. `cargo test`).
// It allows us to include unit tests directly in the source file next to the code they test.
#[cfg(test)]
pub(crate) mod tests {
    use super::*;
    ...
    // [Jules]
    // `mockall::mock!` is a macro that generates mock structs for testing.
    // These mocks implement the specified traits but allow us to configure their behavior
    // (return values, expectations) in tests.
    mockall::mock! {
        #[derive(Debug)]
        pub RetryThrottler {}
