...
// [Jules] Rust: Import types from other modules within this crate (`crate::`) or from external crates.
...
// [Jules] Rust: `#[derive(Clone, Debug)]` automatically implements `Clone` (allows creating a copy) and `Debug` (allows formatting for debug output) traits for this struct.
// [Jules] Rust: The struct is generic over `S`. The default type for `S` is `crate::stub::DefaultStorage`.
// This allows swapping the underlying implementation, which is useful for testing (mocking).
#[derive(Clone, Debug)]
pub struct Storage<S = crate::stub::DefaultStorage>
where
    // [Jules] Rust: This `where` clause specifies constraints on the generic type `S`.
    // `S` must implement the `crate::stub::Storage` trait and have a `'static` lifetime (meaning it doesn't contain any non-static references).
    S: crate::stub::Storage + 'static,
{
    // [Jules] Rust: `Arc` (Atomic Reference Counted) smart pointer.
    // It allows multiple parts of the application to share ownership of the `stub`.
    // The `stub` handles the actual communication (HTTP/gRPC).
    stub: std::sync::Arc<S>,
...
pub(crate) struct StorageInner {
    // [Jules] SDK: `gaxi` (Google API Extensions) provides the HTTP and gRPC clients.
...
// Note that here `S` is not specified, so these methods apply to `Storage` with the default generic parameter (or any `S`? No, without generic params, it implies specific types or needs checking. Actually, usually `impl Storage` implies `Storage<DefaultStorage>` if `S` has a default. But let's check if `builder` depends on `S`? No, it returns `ClientBuilder` which is independent of `S`. So this block is for the struct regardless of `S`? No, strictly `impl Storage` means `impl Storage<DefaultStorage>`.)
// Wait, `ClientBuilder` is independent.
impl Storage {
...
// [Jules] Rust: `impl<S> Storage<S>` defines methods for `Storage` with any type `S` that meets the constraints.
impl<S> Storage<S>
where
    S: crate::storage::stub::Storage + 'static,
{
    ...
    // [Jules] Rust: This method takes `&self` (reference to the client), and generic arguments `B`, `O`, `T`, `P`.
    // `Into<String>` allows passing anything that can be converted into a `String` (like `&str` or `String`).
    // `Into<Payload<P>>` allows passing various types as the payload.
...
// [Jules] Rust: `impl Storage` implies `impl Storage<crate::stub::DefaultStorage>`.
impl Storage {
    // [Jules] Rust: `pub(crate)` means this method is visible only within the crate.
    ...
    pub(crate) async fn new(builder: ClientBuilder) -> BuilderResult<Self> {
        // [Jules] Rust: `.await` suspends execution until the future completes.
        // `?` propagates the error if the result is `Err`.
...
impl StorageInner {
    ...
    // [Jules] Rust: `pub(self)` means private to this module (same as private by default).
    pub(self) fn new(
    ...
    pub(self) async fn from_parts(builder: ClientBuilder) -> BuilderResult<Self> {
        ...
        let cred = config
            .cred
            .clone()
            // [Jules] Rust: `expect` panics with the given message if the value is `None` (or `Err`).
            // Here it's safe because `into_parts` ensures credentials are set.
    ...
    pub async fn apply_auth_headers(
        ...
        let auth_headers = match cached_auth_headers {
            ...
            CacheableResource::NotModified => {
                // [Jules] Rust: `unreachable!` macro indicates code that should never be executed.
                // If it is reached, the program panics.
...
/// # Ok(()) }
/// ```
// [Jules] SDK: `ClientBuilder` is a common pattern in the SDK to configure and create clients.
pub struct ClientBuilder {
    // Common options for all clients (generated or not).
    // [Jules] SDK: `ClientConfig` holds common configuration like retry policies, credentials, etc.
...
impl ClientBuilder {
    ...
    /// # Ok(()) }
    /// ```
    // [Jules] Rust: `self` (consuming) means the builder is consumed when `build` is called.
    // You cannot use the builder again after this.
...
pub(crate) fn apply_customer_supplied_encryption_headers(
    ...
    // [Jules] Rust: `iter()` creates an iterator.
    // `fold` accumulates a value by applying a function to each element.
    // Here it applies headers to the builder for each param (if any).
...
// [Jules] Rust: `#[cfg(test)]` means this module is only compiled when running tests.
#[cfg(test)]
pub(crate) mod tests {
    use super::*;
    ...
    // [Jules] Rust: `mockall::mock!` generates a mock implementation of a struct or trait.
    // This is used for unit testing to isolate the code being tested from its dependencies.
    mockall::mock! {
        #[derive(Debug)]
        pub RetryThrottler {}
