...
// RUST: Importing items from modules within the same crate using `crate::`.
...
// RUST: Importing external crates. `base64` is used for encoding keys.
use base64::Engine;
...
// RUST: `#[derive(Clone, Debug)]` automatically implements the `Clone` and `Debug` traits.
// `Clone` allows creating a duplicate of the struct (cheaply here, due to Arc).
// `Debug` allows printing the struct for debugging purposes using `{:?}`.
//
// SDK: The `Storage` struct is the main entry point for interacting with GCS.
// It is generic over `S`, which represents the underlying storage transport/stub.
// By default, it uses `crate::stub::DefaultStorage`.
#[derive(Clone, Debug)]
pub struct Storage<S = crate::stub::DefaultStorage>
where
    // RUST: The `where` clause specifies constraints on the generic type `S`.
    // `S` must implement the `crate::stub::Storage` trait and must be valid for the `'static` lifetime.
    S: crate::stub::Storage + 'static,
{
    // RUST: `Arc` (Atomic Reference Counted) is a thread-safe reference-counting pointer.
    // This allows the `stub` to be shared across threads and multiple `Storage` instances cheaply.
    stub: std::sync::Arc<S>,
    options: RequestOptions,
}

// RUST: `pub(crate)` means this struct is visible within the crate but not to external users.
//
// SDK: `StorageInner` holds the actual clients (HTTP and gRPC) and configuration.
// It is separated to keep the public `Storage` struct clean and to facilitate sharing state.
#[derive(Clone, Debug)]
...
// RUST: This `impl` block defines methods associated with the `Storage` struct,
// regardless of the generic type `S`.
impl Storage {
    ...
    /// # Ok(()) }
    /// ```
    // RUST: This is a "constructor" method that returns a `ClientBuilder`.
    // It is a static method (no `&self` argument).
    pub fn builder() -> ClientBuilder {
        ClientBuilder::new()
    }
}

// RUST: This `impl` block defines methods specifically for `Storage<S>` where `S` implements
// `crate::storage::stub::Storage`.
impl<S> Storage<S>
where
    S: crate::storage::stub::Storage + 'static,
{
    ...
    /// client's behavior.
    // RUST: This allows creating a `Storage` client with a custom stub, useful for testing.
    ...
    // RUST: This method uses generics `B`, `O`, `T`, `P` to accept various types
    // as arguments, as long as they can be converted (`Into`) to the required types.
    // This makes the API flexible (e.g., accepting `String` or `&str`).
    ...
    ///   `projects/_/buckets/{bucket_id}` format.
    /// * `object` - the object name.
    // RUST: Returns a `ReadObject` builder. The actual network request happens when
    // `send()` is called on the returned builder.
...
// RUST: This `impl` block is for the `Storage` struct with default generic parameters.
impl Storage {
    // RUST: `pub(crate)` method, used internally to construct the `Storage` from a `ClientBuilder`.
    // It's `async` because it involves initializing network clients which might perform I/O.
    pub(crate) async fn new(builder: ClientBuilder) -> BuilderResult<Self> {
        let inner = StorageInner::from_parts(builder).await?;
        let options = inner.options.clone();
        // SDK: Initializes the transport layer with the inner client.
...
impl StorageInner {
    ...
    // RUST: `pub(self)` means private to this module.
    pub(self) fn new(
    ...
    pub(self) async fn from_parts(builder: ClientBuilder) -> BuilderResult<Self> {
        ...
        // RUST: `expect` unwraps the Option, panicking with the message if it's None.
        // Here it's safe because `into_parts` guarantees credentials are set.
        let cred = config
            .cred
            .clone()
    ...
    pub async fn apply_auth_headers(
        ...
        // SDK: Retrieves cached authentication headers or fetches new ones.
        ...
        let auth_headers = match cached_auth_headers {
            ...
            CacheableResource::NotModified => {
                // RUST: `unreachable!` macro indicates code that should never be executed.
                unreachable!("headers are not cached");
            }
        };

        // SDK: Adds the authentication headers to the request builder.
...
/// # Ok(()) }
/// ```
// RUST: The Builder pattern is used to construct complex objects step-by-step.
// It's very common in Rust for configuration.
...
impl ClientBuilder {
    pub(crate) fn new() -> Self {
        // SDK: Sets default configuration values (retry, backoff, parallelism).
    ...
    /// # Ok(()) }
    /// ```
    // RUST: Consumes the builder (`self`) and returns the built `Storage` object.
    // It's async because initialization might involve I/O.
    ...
    /// # Ok(()) }
    /// ```
    // RUST: Takes `mut self` to modify the builder in place and returns `Self` for method chaining.
...
// RUST: `const` items are evaluated at compile time and inlined where used.
// `percent_encoding::AsciiSet` is efficient for checking if a char needs encoding.
...
pub(crate) fn apply_customer_supplied_encryption_headers(
    ...
    // RUST: `iter()` creates an iterator over the Option (yields 0 or 1 item).
    // `fold` accumulates a value (the `builder` here) by applying a closure.
...
// RUST: `#[cfg(test)]` means this module is only compiled when running tests (`cargo test`).
#[cfg(test)]
pub(crate) mod tests {
    use super::*;
    ...
    // RUST: `mockall::mock!` is a macro for creating mock objects for testing.
    mockall::mock! {
        #[derive(Debug)]
        pub RetryThrottler {}
