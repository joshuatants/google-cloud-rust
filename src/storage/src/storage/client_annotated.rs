...
// [Jules: Rust] `use` declarations import items from other modules into the current scope.
// [Jules: Rust] `super::` refers to the parent module (`src/storage/src/storage/mod.rs` or directory).
// [Jules: Rust] `crate::` refers to the root of the current crate (`src/storage/src/lib.rs`).
...
// [Jules: Rust] `#[derive(Clone, Debug)]` automatically implements `Clone` and `Debug` traits for the struct.
// [Jules: Rust] `Clone` allows creating a copy of the client. Since the internal state is wrapped in `Arc` (Atomic Reference Counted),
// [Jules: Rust] cloning is cheap (it just increments a reference counter).
// [Jules: Rust] The generic parameter `<S = crate::stub::DefaultStorage>` allows injecting a different storage implementation,
// [Jules: Rust] primarily for testing (mocking). It defaults to `DefaultStorage` so users don't need to specify it.
#[derive(Clone, Debug)]
pub struct Storage<S = crate::stub::DefaultStorage>
where
    // [Jules: Rust] The `where` clause specifies constraints on the generic type `S`.
    // [Jules: Rust] `S` must implement `crate::stub::Storage` and have a static lifetime (`'static`).
    // [Jules: Rust] `'static` means the type does not contain any non-static references.
    S: crate::stub::Storage + 'static,
{
    // [Jules: Rust] `Arc` is a thread-safe reference-counting pointer. This allows the `stub` to be shared across threads.
    stub: std::sync::Arc<S>,
    options: RequestOptions,
}

// [Jules: Rust] `pub(crate)` makes this struct visible only within the crate, not to external users.
// [Jules: SDK] `StorageInner` holds the actual HTTP and gRPC clients, credentials, and options.
// [Jules: SDK] It's wrapped in an `Arc` inside `DefaultStorage` (which `Storage` holds).
#[derive(Clone, Debug)]
pub(crate) struct StorageInner {
    // [Jules: SDK] We use `reqwest` for HTTP requests.
    pub client: gaxi::http::ReqwestClient,
    pub cred: Credentials,
    pub options: RequestOptions,
    // [Jules: SDK] We also have a gRPC client, though most Storage operations currently use HTTP/JSON.
    pub grpc: gaxi::grpc::Client,
}

// [Jules: Rust] `impl Storage` defines methods associated with the `Storage` struct.
// [Jules: Rust] These methods apply to `Storage` with the default generic parameter (`DefaultStorage`).
impl Storage {
...
// [Jules: Rust] This `impl` block defines methods for `Storage<S>` where `S` implements the `Storage` trait.
// [Jules: Rust] This covers both the default client and any mocked clients.
impl<S> Storage<S>
where
    S: crate::storage::stub::Storage + 'static,
{
    ...
    where
        // [Jules: Rust] Redundant bound specification, but harmless. Ensures `S` is compatible with the inner stub requirement.
    ...
    // [Jules: Rust] This function uses generics `B`, `O`, `T`, `P`.
    // [Jules: Rust] `B` and `O` must convert into `String` (for bucket and object names).
    // [Jules: Rust] `T` must convert into `Payload<P>`, where `P` is the type of the underlying data source.
    // [Jules: SDK] This creates a `WriteObject` builder, which allows setting more options before executing the request.
    ...
    ///   `projects/_/buckets/{bucket_id}` format.
    /// * `object` - the object name.
    // [Jules: SDK] Creates a `ReadObject` builder for reading object data.
    ...
    ///   `projects/_/buckets/{bucket_id}` format.
    /// * `object` - the object name.
    // [Jules: SDK] `open_object` allows more advanced reading patterns, like concurrent ranged reads.
...
// [Jules: Rust] Another `impl` block for `Storage` (default type parameters).
// [Jules: Rust] `pub(crate)` restricts `new` to be called only within this crate.
// [Jules: Rust] The public way to create a client is via `Storage::builder()`.
impl Storage {
...
impl StorageInner {
    ...
    // [Jules: Rust] `pub(self)` means private to this module (same as just `fn new(...)`).
    pub(self) fn new(
    ...
    // [Jules: SDK] This method handles the logic of injecting the correct authentication tokens (e.g., OAuth2 Bearer tokens)
    // [Jules: SDK] into the HTTP request headers.
...
/// # Ok(()) }
/// ```
// [Jules: Rust] The Builder pattern is used here to construct the `Storage` client.
// [Jules: Rust] It allows setting optional configuration parameters before creating the final object.
...
impl ClientBuilder {
    pub(crate) fn new() -> Self {
        ...
        {
            // [Jules: Rust] `std::thread::available_parallelism()` attempts to estimate the number of available CPU cores.
    ...
    /// # Ok(()) }
    /// ```
    // [Jules: Rust] `async fn` indicates this function returns a `Future`. It must be `await`ed.
    // [Jules: Rust] Creating the client involves async operations (like discovering default credentials).
    ...
    /// # Ok(()) }
    /// ```
    // [Jules: Rust] `mut self` allows the method to modify the builder instance and return it.
    // [Jules: Rust] This enables method chaining (fluent interface).
    // [Jules: Rust] `V: Into<String>` allows passing anything that can be converted to a String (e.g., `&str`, `String`).
    ...
    // [Jules: SDK] Resumable uploads are a Google Cloud Storage feature that allows resuming an upload
    // [Jules: SDK] if the connection breaks, which is important for large files.
    ...
    /// # Ok(()) }
    /// ```
    // [Jules: SDK] `ReadResumePolicy` determines when and how to resume interrupted downloads.
    ...
    pub(crate) fn apply_default_credentials(&mut self) -> BuilderResult<()> {
        ...
        // [Jules: SDK] If no credentials are provided, we look for Application Default Credentials (ADC).
        // [Jules: SDK] This usually means checking environment variables like `GOOGLE_APPLICATION_CREDENTIALS`
        // [Jules: SDK] or checking the metadata server if running on Google Cloud.
    ...
    pub(crate) fn apply_default_endpoint(&mut self) -> BuilderResult<()> {
        // [Jules: Rust] `get_or_insert_with` computes the value only if the Option is None.
        let _ = self
            .config
            .endpoint
...
// [Jules: Rust] `const` defines a constant value evaluated at compile time.
...
// [Jules: SDK] Customer-Supplied Encryption Keys (CSEK) allow you to provide your own encryption keys.
// [Jules: SDK] The client library sends these keys in the request headers so Google can use them
// [Jules: SDK] to encrypt/decrypt your data.
pub(crate) fn apply_customer_supplied_encryption_headers(
    ...
    // [Jules: Rust] `iter()` creates an iterator. `fold` accumulates a value (the builder) by applying a function
    // [Jules: Rust] to each element. Here, it adds headers if `common_object_request_params` is `Some`.
...
// [Jules: Rust] `#[cfg(test)]` means this module is only compiled when running tests (`cargo test`).
#[cfg(test)]
pub(crate) mod tests {
    use super::*;
    ...
    fn default_settings() {
        ...
        // [Jules: Rust] `assert!` panics if the condition is false, failing the test.
    ...
    // [Jules: Rust] `mockall::mock!` is a macro from the `mockall` crate that generates mock implementations of traits.
    // [Jules: Rust] This is very useful for unit testing components that depend on these traits.
    mockall::mock! {
        #[derive(Debug)]
        pub RetryThrottler {}
