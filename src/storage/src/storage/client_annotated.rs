...
// Rust: `super` refers to the parent module. `crate` refers to the root of the current crate.
// Using `crate::` paths is generally preferred for clarity, except for tightly coupled submodules.
...
// `Clone` allows creating a copy of the struct (in this case, a cheap copy because it contains an `Arc`).
// `Debug` allows printing the struct using `{:?}`.
//
// Generic `S`: This struct is generic over `S`, which defaults to `crate::stub::DefaultStorage`.
// This pattern allows swapping the implementation of the storage backend, which is crucial for testing (mocking).
//
// Trait Bound: `where S: crate::stub::Storage + 'static` enforces that `S` must implement the `Storage` trait
// and live for the `'static` lifetime (basically, own its data or live for the entire program execution).
#[derive(Clone, Debug)]
pub struct Storage<S = crate::stub::DefaultStorage>
where
    S: crate::stub::Storage + 'static,
{
    // Rust: `Arc` (Atomic Reference Counted) smart pointer.
    // It allows multiple owners of the same data, thread-safe.
    // This is why `Storage` is cheap to clone; you're just incrementing a counter, not copying the underlying connection.
    stub: std::sync::Arc<S>,
    options: RequestOptions,
}

// Rust: `pub(crate)` makes this struct visible only within the crate.
// It holds the actual client state.
#[derive(Clone, Debug)]
pub(crate) struct StorageInner {
    // SDK: `ReqwestClient` handles HTTP requests.
    pub client: gaxi::http::ReqwestClient,
    // SDK: `Credentials` handles authentication tokens.
    pub cred: Credentials,
    pub options: RequestOptions,
    // SDK: `grpc::Client` handles gRPC requests.
    pub grpc: gaxi::grpc::Client,
}

// Rust: Implementation block for `Storage`.
// Note that this block applies to `Storage` with default generic `S`.
impl Storage {
...
// Rust: Implementation block for `Storage` with any generic `S` that satisfies the bounds.
impl<S> Storage<S>
where
    S: crate::storage::stub::Storage + 'static,
{
    ...
    // Rust: Generics `B`, `O`, `T`, `P`.
    // `Into<String>` allows passing `String`, `&str`, etc., which are then converted to `String`.
    // `Into<Payload<P>>` allows flexibility in what data can be uploaded.
    // `self` is passed by reference, meaning the method doesn't consume the client.
    ...
    where
        ...
        T: Into<Payload<P>>,
    {
        // SDK: Returns a `WriteObject` builder which allows further configuration before sending.
        WriteObject::new(
            self.stub.clone(),
            bucket,
            object,
    ...
    where
        ...
        O: Into<String>,
    {
         // SDK: Returns a `ReadObject` builder.
...
// Rust: Implementation specific to `Storage` with default `S` (the production client).
impl Storage {
    // Rust: `pub(crate)` allows only internal access.
    // `async fn` indicates this function performs asynchronous operations and returns a `Future`.
    pub(crate) async fn new(builder: ClientBuilder) -> BuilderResult<Self> {
        let inner = StorageInner::from_parts(builder).await?;
        let options = inner.options.clone();
        // SDK: Wraps `StorageInner` into a `stub::Storage` implementation for production use.
...
impl StorageInner {
    ...
    // Rust: `pub(self)` is equivalent to `private`.
    pub(self) fn new(
    ...
    pub(self) async fn from_parts(builder: ClientBuilder) -> BuilderResult<Self> {
        // Rust: Deconstructs the builder to get config and options.
        // `?` propagates errors.
        let (mut config, options) = builder.into_parts()?;
        config.disable_automatic_decompression = true;
        // Rust: `expect` will panic if the value is None, but here we know it should be set by `into_parts`.
        let cred = config
            .cred
            .clone()
    ...
    pub async fn apply_auth_headers(
        ...
        // SDK: Retrieves cached authentication headers (e.g. Bearer token).
        ...
        // SDK: `CacheableResource` optimization to reuse headers if not expired.
...
/// # Ok(()) }
/// ```
// Rust: Builder pattern allows constructing complex objects step-by-step.
...
impl ClientBuilder {
    pub(crate) fn new() -> Self {
        ...
        // SDK: Setting default retry and backoff policies.
        ...
        {
             // Rust: Getting available parallelism (CPU cores) to optimize channel count.
    ...
    /// # Ok(()) }
    /// ```
    // Rust: `self` consumes the builder, so it can't be used again.
    ...
    /// # Ok(()) }
    /// ```
    // Rust: `mut self` allows chaining methods (fluent interface).
    ...
    pub(crate) fn apply_default_credentials(&mut self) -> BuilderResult<()> {
        ...
        // SDK: Loads Application Default Credentials (ADC) if none provided.
    ...
    pub(crate) fn apply_default_endpoint(&mut self) -> BuilderResult<()> {
        // Rust: `get_or_insert_with` computes the value only if the Option is None.
        let _ = self
            .config
            .endpoint
...
// Rust: `const` declares a constant value.
// `percent_encoding` is a crate used for URL encoding.
...
// SDK: Helper to add customer-supplied encryption headers.
pub(crate) fn apply_customer_supplied_encryption_headers(
    ...
    // Rust: `iter()` allows iterating over Option (0 or 1 element).
    // `fold` accumulates the result, starting with `builder`.
...
// Rust: `#[cfg(test)]` indicates this module is only compiled for testing.
#[cfg(test)]
pub(crate) mod tests {
    use super::*;
    ...
    // Rust: `mockall::mock!` macro generates a mock implementation of a struct or trait.
    // This is used for unit testing where you want to simulate dependencies.
    mockall::mock! {
        #[derive(Debug)]
        pub RetryThrottler {}
