...
// [Jules: Rust] Imports from the parent module.
use super::Reader;
...
// [Jules: SDK] This struct wraps the HTTP response and handles the logic for:
// [Jules: SDK] 1. Parsing the response headers (generation, size, checksums).
// [Jules: SDK] 2. Reading the body chunks.
// [Jules: SDK] 3. Validating checksums when the download completes.
// [Jules: SDK] 4. Automatically resuming the download if the connection is interrupted.
/// A response to a [Storage::read_object] request.
#[derive(Debug)]
pub(crate) struct ResumableResponse {
    // [Jules: SDK] We need the `Reader` to trigger new requests if we need to resume.
    reader: Reader,
    // [Jules: SDK] The current active HTTP response.
    response: Option<Response>,
    // [Jules: SDK] Metadata about the object being read.
    ...
    // [Jules: SDK] Tracks how much data is left to read.
    range: ReadRange,
    // [Jules: SDK] The generation of the object we are reading. Crucial for resuming to ensure consistency.
    generation: i64,
    resume_count: u32,
}
...
impl ResumableResponse {
    pub(crate) fn new(reader: Reader, response: Response) -> Result<Self> {
        // [Jules: SDK] Check if the user requested the full object (offset 0, no limit).
        let full = reader.request.read_offset == 0 && reader.request.read_limit == 0;
        let headers = response.headers();
        // [Jules: SDK] Determine if we should validate checksums based on headers and request type.
        let response_checksums = checksums_from_response(full, response.status(), headers);
        // [Jules: SDK] Parse the range of data returned by this response.
        let range = response_range(&response).map_err(Error::deser)?;
        // [Jules: SDK] Parse the object generation.
        let generation =
...
// [Jules: Rust] `#[async_trait::async_trait]` is needed because traits with async methods are not yet fully supported in stable Rust without boxing.
...
impl crate::read_object::dynamic::ReadObjectResponse for ResumableResponse {
    ...
    // [Jules: SDK] Returns the next chunk of data.
    async fn next(&mut self) -> Option<Result<bytes::Bytes>> {
        match self.next_attempt().await {
            ...
            // [Jules: Rust] If an error occurs, we try to resume. `Box::pin` is required for recursive async calls.
...
impl ResumableResponse {
    // [Jules: SDK] Tries to read the next chunk from the *current* response.
    async fn next_attempt(&mut self) -> Option<Result<bytes::Bytes>> {
        let response = self.response.as_mut()?;
        // [Jules: Rust] Read a chunk from the reqwest response stream.
        let res = response.chunk().await.map_err(Error::io);
        match res {
            Ok(Some(chunk)) => {
                // [Jules: SDK] Update the running checksum with the new data.
                self.reader
                    .options
                    .checksum
                    .update(self.range.start, &chunk);
                let len = chunk.len() as u64;
                // [Jules: SDK] Sanity check: did we get more data than expected?
                ...
                // [Jules: SDK] Update the range tracking.
                self.range.limit -= len;
                self.range.start += len;
                Some(Ok(chunk))
            }
            Ok(None) => {
                // [Jules: SDK] End of stream.
                // [Jules: SDK] If we haven't received all expected bytes, it's a short read error.
                if self.range.limit != 0 {
                    return Some(Err(Error::io(ReadError::ShortRead(self.range.limit))));
                }
                // [Jules: SDK] Verify checksums.
    ...
    // [Jules: SDK] Resumes the download after an error.
    async fn resume(&mut self, error: Error) -> Option<Result<bytes::Bytes>> {
        ...
        // [Jules: SDK] Check the resume policy to see if we should retry.
        match self
            .reader
            .options
            .read_resume_policy()
        ...
        // [Jules: SDK] Update the request to start from the current offset tracked in `self.range.start`.
        self.reader.request.read_offset = self.range.start as i64;
        self.reader.request.read_limit = self.range.limit as i64;
        // [Jules: SDK] Ensure we request the exact same generation of the object.
        self.reader.request.generation = self.generation;
        // [Jules: SDK] Initiate a new request.
        self.response = match self.reader.clone().read().await {
            Ok(r) => Some(r),
            Err(e) => return Some(Err(e)),
        };
        // [Jules: SDK] Call `next` again (which will call `next_attempt` on the new response).
        self.next().await
    }
}
...
// [Jules: SDK] Helper to determine the range of data provided by the response.
fn response_range(response: &Response) -> std::result::Result<ReadRange, ReadError> {
    match response.status() {
        StatusCode::OK => {
            // [Jules: SDK] 200 OK means we got the full content (or what was requested if no range was specified).
            match (
        ...
        StatusCode::PARTIAL_CONTENT => {
            // [Jules: SDK] 206 Partial Content means we got a range. Parse `Content-Range`.
...
/// create it,
// [Jules: SDK] Logic to determine if we can validate checksums.
fn checksums_from_response(
    ...
    // [Jules: SDK] If we only got a part of the file, the object-level checksum won't match the part.
    if !full_content_requested || status == http::StatusCode::PARTIAL_CONTENT {
        return checksums;
    }
    // [Jules: SDK] Check for server-side transcoding/compression.
    ...
    // [Jules: SDK] Extract checksums from headers.
    checksums
