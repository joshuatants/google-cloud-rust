...
// [Jules: SDK] Helper functions for handling routing parameters.
// [Jules: SDK] Routing parameters are values extracted from the request (e.g., bucket name) and added to headers
// [Jules: SDK] to help the Google Cloud load balancers route the request to the correct backend.
...
// [Jules: SDK] Extracts a specific segment from a resource name based on a path template.
// [Jules: SDK] The template is split into `prefix` (before the interesting part), `matching` (the part we want), and `suffix` (after).
pub fn value<'h>(
    ...
    // [Jules: Rust] `?` on Option returns `None` early if the value is `None`.
    let haystack = haystack?; // Consuming Option<> simplifies code generation
    let mut remains = haystack;
    let mut start = 0_usize;
    let mut end = 0_usize;

    // [Jules: Rust] Iterate through prefix segments to advance the start pointer.
    ...
    // [Jules: Rust] Iterate through matching segments to advance the end pointer.
    for needle in matching {
        let count = needle.match_size(remains)?;
        end += count;
        remains = &remains[count..];
    }
    // [Jules: Rust] Verify suffix matches.
    for needle in suffix {
        let count = needle.match_size(remains)?;
        remains = &remains[count..];
    }
    // [Jules: Rust] If there are leftover characters, the template didn't match the full string.
    if !remains.is_empty() || start == end {
        return None;
    }
    // [Jules: Rust] Return the slice corresponding to the `matching` part.
...
/// ```
// [Jules: SDK] Formats the extracted parameters into a string suitable for the `x-goog-request-params` header.
// [Jules: SDK] The format is `key1=value1&key2=value2`, with values URL-encoded.
...
// [Jules: SDK] Enum defining the different types of path segments used in AIP-4222 templates.
pub enum Segment {
...
impl Segment {
    // [Jules: Rust] Logic to calculate the length of the match for a given segment type.
    pub(crate) fn match_size(&self, haystack: &str) -> Option<usize> {
        match self {
            Self::Literal(lit) => haystack.starts_with(lit).then_some(lit.len()),
            Self::SingleWildcard => {
                // [Jules: SDK] Single wildcard matches until the next slash or end of string.
...
// [Jules: Rust] `percent_encoding` crate helper set.
