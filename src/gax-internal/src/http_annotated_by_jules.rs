...
// [Jules: SDK] This module implements the HTTP client used by the Google Cloud client libraries.
// [Jules: SDK] It wraps `reqwest::Client` to provide authentication, retries, and other cloud-specific features.
...
// [Jules: Rust] `pub mod` makes the `reqwest` module public, re-exporting it.
pub mod reqwest;

// [Jules: Rust] Imports from other modules in the crate or dependencies.
...
// [Jules: SDK] The `ReqwestClient` struct holds the state for the HTTP client.
// [Jules: Rust] `#[derive(Clone, Debug)]` allows the client to be cheaply cloned (shared) and printed for debugging.
#[derive(Clone, Debug)]
pub struct ReqwestClient {
    // [Jules: SDK] The underlying `reqwest::Client` that performs the actual network I/O.
    inner: ::reqwest::Client,
    // [Jules: SDK] The credentials used to sign requests.
    cred: Credentials,
    // [Jules: SDK] The API endpoint (e.g., "https://storage.googleapis.com").
    endpoint: String,
    // [Jules: SDK] The host header value.
    host: String,
    // [Jules: SDK] Policies for retrying failed requests.
    // [Jules: Rust] `Arc<dyn ...>` allows holding any type that implements the trait, sharing it across threads.
    retry_policy: Arc<dyn RetryPolicy>,
    backoff_policy: Arc<dyn BackoffPolicy>,
    retry_throttler: SharedRetryThrottler,
    polling_error_policy: Arc<dyn PollingErrorPolicy>,
    polling_backoff_policy: Arc<dyn PollingBackoffPolicy>,
    // [Jules: SDK] Instrumentation info for tracing/metrics.
    instrumentation: Option<&'static crate::options::InstrumentationClientInfo>,
    _tracing_enabled: bool,
}

impl ReqwestClient {
    // [Jules: Rust] The constructor for `ReqwestClient`. It is `async` because loading credentials might involve I/O.
    pub async fn new(
        config: crate::options::ClientConfig,
        default_endpoint: &str,
    ) -> ClientBuilderResult<Self> {
        // [Jules: SDK] Load credentials based on the configuration.
        let cred = Self::make_credentials(&config).await?;
        // [Jules: SDK] Start building the `reqwest::Client`.
        ...
        if config.disable_follow_redirects {
            // [Jules: Rust] `::reqwest::redirect::Policy::none()` disables automatic redirect following.
            builder = builder.redirect(::reqwest::redirect::Policy::none());
        }
        // [Jules: Rust] Build the client, mapping any errors to `BuilderError::transport`.
        let inner = builder.build().map_err(BuilderError::transport)?;
        // [Jules: SDK] Determine the host from the endpoint configuration.
        ...
        // [Jules: Rust] Construct and return the `ReqwestClient`.
        Ok(Self {
            inner,
            cred,
            endpoint,
            host,
            // [Jules: SDK] Initialize policies, using defaults if not provided in config.
    ...
    // [Jules: SDK] Builder-like method to attach instrumentation info.
    ...
    // [Jules: SDK] Helper to create a request builder pointing to a path relative to the base endpoint.
    ...
    // [Jules: SDK] Used when the full URL is known (e.g. signed URLs or upload URLs).
    pub fn builder_with_url(&self, method: Method, url: &str) -> reqwest::RequestBuilder {
        self.inner.request(method, url)
    }

    // [Jules: SDK] Executes a request with retries and JSON serialization/deserialization.
    // [Jules: Rust] `I` is the input (body) type, `O` is the output (response) type. Both must implement `serde` traits.
    pub async fn execute<I: serde::ser::Serialize, O: serde::de::DeserializeOwned + Default>(
        ...
        // [Jules: SDK] Configure headers.
        builder = self.configure_builder(builder, &options)?;
        if let Some(body) = body {
            // [Jules: SDK] Attach JSON body.
            builder = builder.json(&body);
        }
        // [Jules: SDK] Enter the retry loop.
    ...
    // [Jules: SDK] Executes a request that returns a stream of bytes. No built-in retry logic here.
    pub async fn execute_streaming_once(
        ...
        // [Jules: Rust] Convert `reqwest::Response` to `http::Response`.
        let response = http::Response::from(response);
        let (parts, body) = response.into_parts();
        // [Jules: Rust] Convert the body into a stream of bytes.
    ...
    // [Jules: SDK] Helper to add common headers like User-Agent and Host.
    ...
    // [Jules: SDK] Helper to load credentials if not already provided.
    ...
    // [Jules: SDK] The retry loop manages retrying failed requests based on policies.
    async fn retry_loop<O: serde::de::DeserializeOwned + Default>(
        ...
        // [Jules: Rust] The `inner` closure performs the actual request attempt.
        let inner = async move |d| {
            // [Jules: Rust] We must clone the builder for each attempt because the request consumes it.
            let builder = builder
                .try_clone()
            ...
            let response = this
                ...
                .await?;
            // [Jules: SDK] Convert the raw HTTP response to the typed response `O`.
            self::to_http_response(response).await
        };
        // [Jules: Rust] `sleep` closure used for backoff delays.
        let sleep = async |d| tokio::time::sleep(d).await;
        // [Jules: SDK] Execute the retry loop.
        retry_loop(inner, sleep, idempotent, throttler, retry, backoff).await
    }

    // [Jules: SDK] Performs a single HTTP request.
    async fn request_attempt(
        ...
        // [Jules: Rust] Execute the request using `reqwest`.
        ...
        let intermediate_result = match intermediate_result {
            // [Jules: SDK] If status is not success, convert to a typed HTTP error.
    ...
    // [Jules: SDK] Helpers to get effective policies (from options or defaults).
...
// [Jules: SDK] Maps `reqwest` errors to the SDK's `Error` type.
...
// [Jules: SDK] Marker struct for when no body is needed.
#[derive(Default, serde::Serialize)]
pub struct NoBody;

// [Jules: SDK] Helper to determine if a body should be sent.
...
// [Jules: SDK] GET, PUT, and DELETE are idempotent by default.
pub fn default_idempotency(m: &Method) -> bool {
    m == Method::GET || m == Method::PUT || m == Method::DELETE
}

// [Jules: SDK] Converts a failed `reqwest::Response` into an `Error`, parsing the body for error details.
...
// [Jules: SDK] Converts a successful `reqwest::Response` into a typed `Response<O>`, deserializing the body.
